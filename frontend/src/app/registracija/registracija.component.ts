...
export class RegistracijaComponent implements OnInit {
  ...
  INVALID_CAPTCHA_MESSAGE = 'Potvrdite da niste robot';
  ...
  registracijaPoljoprivrednika() {
    ...
    this.servis.validateCaptcha(this.captchaResponse).subscribe(result => {
      if (result['message'] != 'ok') {
        this.message = this.INVALID_CAPTCHA_MESSAGE;
        this.pressedButton = false;
        return
      }
      this.servis.registracija(this.poljoprivrednik, this.tip.toLowerCase(), 0).subscribe(ret => {
        if (ret['message'].toLowerCase() == 'ok') this.ruter.navigate(['/login']);
  ...
  registracijaPreduzeca() {
    ...
    if (this.emptyField(this.captchaResponse)) {
      this.pressedButton = false;
      this.message = this.INVALID_CAPTCHA_MESSAGE;
      return;
    }
    ...
    this.servis.validateCaptcha(this.captchaResponse).subscribe(result => {
      ...
      this.servis.registracija(this.preduzece, this.tip.toLowerCase(), 0).subscribe(result => {
